...
# -------------------- Settings --------------------
S_list       <- 3                     
n_s          <- 2000                  # per-site total
n_test       <- 2000
alpha_list   <- 0.3                   # fix alpha
epsilon_list <- c(0.75, 1, 2, 3, 4, 5)
h_list       <- seq(0.1, 0.35, 0.05)

# Sensitivity grid (rho varies, Cw fixed)
rho_list <- seq(0, 0.2, by = 0.02)
Cw_fixed <- 0.1
...
out_file <- file.path(outdir, sprintf("rho_sensitivity_run_%03d.txt", run_idx))
header <- "run\tS\tn_s\talpha\tepsilon\th_cv\tM\trho\terr\tabs_DD"
      ...
      for (a_lvl in alpha_list) {
        out_fd <- try(
          FDP_fair_classifier_cross_fit(
            ...
            mu = mu, nu = nu,
            C_K = C_K, Cw = Cw_fixed, rho = rho_val, M = M_dyn, omega = omega, eta = eta,
            pi_method = "empirical"
          ),
          silent = TRUE
        )
        ...
        if (!inherits(out_fd, "try-error")) {
          ...
          cat(sprintf("[warn] rho run=%d S=%d eps=%.3g alpha=%.3g rho=%.3g failed: %s\n",
                      run_idx, S, eps_priv, a_lvl, rho_val, msg), file = stderr())
        }
      }
    }
  }
...
cat("[done] rho sensitivity wrote to:", out_file, "\n")
