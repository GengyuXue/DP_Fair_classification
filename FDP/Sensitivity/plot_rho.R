...
base_cols  <- scale_colour_manual(values = pal_color[eps_present],
                                  name   = "\u03F5",
                                  limits = eps_present, breaks = eps_present)
base_fills <- scale_fill_manual(values = pal_color[eps_present], guide = "none",
                                limits = eps_present)
...
p_err <- ggplot(
  summ_df,
  aes(x = rho, y = err_mean, color = as.factor(epsilon), group = epsilon)
) +
  geom_ribbon(
    aes(x = rho, ymin = err_lo, ymax = err_hi, fill = as.factor(epsilon)),
...
p_dd <- ggplot(
  summ_df,
  aes(x = rho, y = dd_mean, color = as.factor(epsilon), group = epsilon)
) +
  geom_ribbon(
    aes(x = rho, ymin = dd_lo, ymax = dd_hi, fill = as.factor(epsilon)),
    alpha = 0.22, colour = NA, show.legend = FALSE
  ) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.6, shape = 16) +
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(n = 6)) +
  scale_y_continuous(
    limits = c(max(0, dd_lim[1] - 0.02), dd_lim[2] + 0.02),
    breaks = scales::pretty_breaks(n = 6),
    expand = expansion(mult = c(0.02, 0.06))
  ) +
  geom_hline(yintercept = 0.3, linetype = "dashed", color = "grey45", linewidth = 0.6) +
  geom_abline(slope = 1, intercept = 0.3, linetype = "dotdash", color = "grey45", linewidth = 0.6) +
  labs(x = expression(rho), y = "Disparity")
