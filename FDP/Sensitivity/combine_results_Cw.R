...
## -------------------- Discover files --------------------
files <- list.files(outdir, pattern = "^cw_sensitivity_run_\\d+\\.txt$", full.names = TRUE)
if (!length(files)) {
...
## -------------------- Read & validate --------------------
required_cols <- c("run","S","n_s","alpha","epsilon","h_cv","M","Cw","err","abs_DD")
...
cat("Detected:\n  runs (mc): ", mc,
    ...
    "\n  Cw:         ", paste(Cw_list, collapse=", "), "\n", sep="")

## -------------------- Allocate arrays --------------------
# Dimensions: run × S × n_s × alpha × epsilon × Cw
dn <- list(
...
for (i in seq_len(nrow(all_df))) {
  ...
  ci  <- idx(all_df$Cw[i],      Cw_list)
  
  if (any(is.na(c(si,ni,ai,ei,ci)))) next
  
  err_arr[r, si, ni, ai, ei, ci] <- all_df$err[i]
  DD_arr[ r, si, ni, ai, ei, ci] <- all_df$abs_DD[i]
...
combined_rds  <- file.path(outdir, paste0("cw_combined_rows_", timestamp, ".rds"))
