...
read_tab_or_ws <- function(path) {
  # First try strict tab-separated
  ...
  # Fallback: any amount of whitespace
...
for (f in files) {
  ...
  df$epsilon   <- suppressWarnings(as.numeric(df$epsilon))   
...
for (m in method_list) {
  ...
  # summaries over runs (returns matrices indexed by alpha x eps)
...
combined_rds  <- file.path(outdir, paste0("law_unfair_combined_rows_", timestamp, ".rds"))
