...
suppressWarnings(suppressMessages({
  # keep base-R to avoid CLI/rlang overhead
}))
Sys.setenv(
  OMP_NUM_THREADS = "1",
  MKL_NUM_THREADS = "1",
...
run_file <- file.path(outdir, sprintf("adult_unfair_run_%03d.txt", run_idx))
...
prepare_cdp_full_adult <- function(data, scale_X = FALSE) {
  req <- c("Age","Workclass","Education-Num","Sensitive","Label")
  ...
  X <- as.matrix(df[, c("Age","Workclass","Education-Num"), drop = FALSE])
...
if (!exists("as_cdp_input", mode = "function")) {
...
# --- load data ---
dat_path <- file.path(base_dir, "adult.csv")
if (!file.exists(dat_path)) stop("Cannot find adult.csv in working directory")
adult <- read_csv_base(dat_path)
...
tr_full   <- prepare_cdp_full_adult(adult[idx_tr, ],  FALSE)
te_full   <- prepare_cdp_full_adult(adult[idx_te, ],  FALSE)
...
# ---- CDP_unfair once per epsilon; CDP & FDP per (epsilon, alpha) ----
for (eps in epsilon_list) {
  
  # CDP_unfair (DP, no fairness) 
  cdp_unfair_fit <- try(
